...
本仓库为拟投稿 PNAS 论文整理了代码、源数据、代表性 MDS 快照、ESEM 源图、最终补充电影和电影 metadata。目录结构覆盖正文和 SI Appendix 中的主要理论计算、MDS/ESEM 证据以及新增的 CNT 自由能计算。
...
- `data/Supplementary_Movies/`：最终 SI Movies S1-S3，以及图注、源文件来源和 SHA-256 校验值。
- `data/ESEM_movie_metadata/`：ESEM/SI 源图、早期候选 MP4、电影说明、首帧/抽帧预览和 metadata 表，作为整理过程记录保留。
...
最终投稿的 Movies S1-S3 已放入 `data/Supplementary_Movies/`：Movie S1 为不同尺寸双颗粒上的成核，Movie S2 为均一直径多链颗粒聚集体上的成核，Movie S3 为凹球形颗粒上的成核。`data/ESEM_movie_metadata/` 中较早找到的候选 MP4 和预览仅作为整理过程记录保留，不属于最终 SI 电影组。
